...
static std::string core_network_bandwidth = "1Mb/s"; 	//core_network_bandwidth.
...
static std::string init_radio_bandwidth = "1000Mb/s"; 	//radio_link_bandwidth (init).
...
int main (int argc, char *argv[])
{
  ...
  LogComponentEnable("TcpTahoe",level);
    ...
    /**ConfigStore setting*/
    Config::SetDefault("ns3::ConfigStore::Filename", StringValue("emulated-in-up.txt"));
  ...
  //*****************************Install and start applications on UEs and remote host****************************//
   ...
   if (is_tcp == 1){
        				/*********TCP Application********/
       					...
       					serverApps.Add(sink.Install(remote_host));
        				...
        				onOffHelper.SetConstantRate( DataRate(sending_rate), packet_size );
       					clientApps.Add(onOffHelper.Install(ue));
   }
                else{
        					/*********UDP Application********/
  ...
  enb_core_dev->TraceConnect("**EnodeB Core Dev: ", "PhyTxBegin", MakeCallback(&dev_queue_droptail));
  endhost_dev->TraceConnect("**Endhost: ","MacTxDrop", MakeCallback(&dev_queue_droptail)); 
  ue_dev->TraceConnect("**Uehost: ","MacTxDrop", MakeCallback(&dev_queue_droptail)); 

    /****ConfigStore setting****/
    Config::SetDefault("ns3::ConfigStore::Filename", StringValue("emulated-out-up.txt"));
    Config::SetDefault("ns3::ConfigStore::FileFormat", StringValue("RawText"));
