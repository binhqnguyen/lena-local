...
//       LogComponentEnable ("LtePhy", level);
    ...
    if (isPedestrian == 1 || isVehicular == 1){
      ...
      lteHelper->SetFadingModelAttribute("SamplesNum",UintegerValue(10000));  /*1sample/1ms*/
      lteHelper->SetFadingModelAttribute("WindowSize",TimeValue(Seconds(0.5)));
      lteHelper->SetFadingModelAttribute("RbNum",UintegerValue(100));
    }
    if(isPedestrian==1)
        lteHelper->SetFadingModelAttribute("TraceFilename", StringValue("/Users/binh/Documents/workspace/lena/src/lte/model/fading-traces/fading_trace_EPA_3kmph.fad"));
    if(isVehicular==1)
...
getTcpPut(Ptr<LteHelper> lteHelper){
    ...
    for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator iter = stats.begin(); iter != stats.end(); ++iter){
			...
			/*for UL experiment, acking flow is flow from 7.0.0.2 to 1.0.0.2*/
			if (t.sourceAddress.IsEqual(Ipv4Address("7.0.0.2"))){
				if (iter->second.rxPackets > 1){
					meanTxRate_1 =  8*iter->second.txBytes/(iter->second.timeLastTxPacket-iter->second.timeFirstTxPacket)*1000000000/(1024);
					meanRxRate_1 = 8*iter->second.rxBytes/(iter->second.timeLastRxPacket-iter->second.timeFirstRxPacket)*1000000000/(1024);
					meanTcpDelay_1 = iter->second.delaySum/iter->second.rxPackets/1000000;
				}
				numOfLostPackets_1 = iter->second.lostPackets;
				numOfTxPacket_1 = iter->second.txPackets;
			...
			/*for DL experiment, sending flow is flow from 1.0.0.2 to 7.0.0.2*/
			if (t.sourceAddress.IsEqual(Ipv4Address("1.0.0.2"))){
				if (iter->second.rxPackets > 1){
					meanTxRate_2 =  8*iter->second.txBytes/(iter->second.timeLastTxPacket-iter->second.timeFirstTxPacket)*1000000000/(1024);
					meanRxRate_2 = 8*iter->second.rxBytes/(iter->second.timeLastRxPacket-iter->second.timeFirstRxPacket)*1000000000/(1024);
					meanTcpDelay_2 = iter->second.delaySum/iter->second.rxPackets/1000000;
				}
				numOfLostPackets_2 = iter->second.lostPackets;
				numOfTxPacket_2 = iter->second.txPackets;
    ...
    tcpThroughput << Simulator::Now().GetMilliSeconds() << "\t\t"
                << meanRxRate_2.GetDouble() << "\t\t"
                << meanTcpDelay_2.GetDouble() << "\t\t"
                << numOfLostPackets_2 << "\t\t"
                << numOfTxPacket_2 << "\t\t"
                << errorUlRx << "\t\t"
                << errorDlRx << "\t\t"
