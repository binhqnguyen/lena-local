 ...
 */
NS_LOG_COMPONENT_DEFINE ("e2e-tcp-ul");
...
//       LogComponentEnable ("LteRlcUm", level);
    ...
    double simTime = 1;	//simulation time for EACH application
    ...
    /**ConfigStore setting*/
    Config::SetDefault("ns3::ConfigStore::Filename", StringValue("config-in-tcp.txt"));
...
//        clientApps.Add (ulClient.Install (ueNodes.Get(u)));	//install the client to the Ue node.
    ...
    for ( uint32_t i = 0 ; i < enbNodes.GetN(); ++i){
        for ( uint32_t j = 0; j < ueNodes.GetN(); ++j){
            ...
            std::cout << std::left << std::setw(SPC) << double (lteHelper->GetPdcpStats()->GetUlDelay(j+1,3))*1000/ONEBIL;
            std::cout << std::left << std::setw(SPC) << lteHelper->GetPdcpStats()->GetDlDelay(j+1,3)*1000/ONEBIL;
            std::cout << std::left << std::setw(SPC) << lteHelper->GetRlcStats()->GetUlDelay(j+1,3)*1000/ONEBIL;
            std::cout << std::left << std::setw(SPC) << lteHelper->GetRlcStats()->GetDlDelay(j+1,3)*1000/ONEBIL;
    ...
    oFile << appRate << "   ";
    //UL PDCP PDUs
    oFile << double (lteHelper->GetPdcpStats()->GetUlDelay(1,3)*1000/ONEBIL) << " ";
    //UL RLC PDUs
    oFile << double (lteHelper->GetRlcStats()->GetUlDelay(1,3)*1000/ONEBIL) << "\n";
